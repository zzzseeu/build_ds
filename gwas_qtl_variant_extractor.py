...
class GWASQTLVariantExtractor:
    ...
    def run(self) -> pd.DataFrame:
        ...
        site_df = self._extract_from_vcf(gwas_site_map, qtl_trees)
        site_df = self._filter_sites_by_feature_trees(site_df, feature_trees)
        site_df = self._filter_by_gene_list(site_df)
        geno_extractor = GWASQTLGenotypeExtractor(
            vcf_path=str(self.vcf_path),
            site_df=site_df,
            outprefix=None,
        )
        out_df = geno_extractor.run()
...
class GWASQTLGenotypeExtractor:
    ...
    The input site table is expected to be the output of
    ``GWASQTLVariantExtractor.run()``, with columns:
    ``Chromosome, Position, Gene, gwas_trait, qtl_trait``.
    ...
    def __init__(
        ...
        log_path = log_dir / f"{log_stem}_genotype_{datetime.now().strftime('%Y-%m-%d')}.log"
    ...
    def _load_site_df(self) -> pd.DataFrame:
        required = ["Chromosome", "Position", "Gene", "gwas_trait", "qtl_trait"]
        ...
        self.logger.info("Loaded site_df: shape=%s unique_sites=%d", df.shape, df[["Chromosome", "Position"]].drop_duplicates().shape[0])
        return df
    ...
    def run(self) -> pd.DataFrame:
        ...
        for rec in vcf:
            ...
            if matched % 1000 == 0 or matched == len(target_sites):
                self.logger.info("VCF genotype extraction progress: matched=%d/%d seen=%d", matched, len(target_sites), seen)
        ...
        out_df = pd.DataFrame(rows, columns=["Chromosome", "Position", "Gene", "gwas_trait", "qtl_trait"] + samples)
        self.logger.info("Built genotype matrix: shape=%s", out_df.shape)

        if self.outprefix is not None:
            date_tag = datetime.now().strftime("%Y-%m-%d")
            out_csv = self.outprefix.parent / f"{self.outprefix.name}_genotype_{date_tag}.csv"
            out_df.to_csv(out_csv, index=False)
